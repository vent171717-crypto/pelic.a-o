...
.logo{color:var(--p);font-weight:700;font-size:18px;cursor:pointer;padding:4px 8px;border-radius:4px;transition:background 0.1s}
.logo:hover,.logo.f{background:rgba(245,197,24,0.1)}
.srch{flex:1;background:var(--bg);border:2px solid var(--b);color:var(--t);padding:10px;border-radius:8px;font-size:16px;outline:none;transition:border-color 0.15s}
.srch:focus,.srch.f{border-color:var(--p)}
.btn{background:var(--c);border:2px solid var(--b);color:var(--t);padding:10px 16px;border-radius:8px;font-weight:600;cursor:pointer;transition:all 0.15s}
.btn:hover,.btn.f{background:var(--p);color:#000;border-color:var(--p)}
.stats{color:var(--t2);font-size:12px;margin-left:auto}
.main{flex:1;overflow-y:auto;padding:8px;-webkit-overflow-scrolling:touch}
.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));gap:6px}
.card{position:relative;aspect-ratio:2/3;background:var(--c);border-radius:6px;overflow:hidden;border:2px solid transparent;cursor:pointer;transition:transform 0.1s, border-color 0.1s}
.card:hover{transform:scale(1.01)}
.card.f{border-color:var(--p);transform:scale(1.03);box-shadow:0 0 12px rgba(245,197,24,.2);z-index:10}
.card img{width:100%;height:100%;object-fit:cover;background:linear-gradient(45deg,#1a1a1a 25%,#222 25%,#222 50%,#1a1a1a 50%,#1a1a1a 75%,#222 75%,#222);background-size:20px 20px;opacity:0;transition:opacity 0.2s ease-in-out}
.card img.loaded{opacity:1}
.card-t{position:absolute;bottom:0;left:0;right:0;padding:20px 6px 6px;background:linear-gradient(transparent,#000);font-size:11px;font-weight:600;opacity:0;transform:translateY(5px);transition:opacity 0.15s, transform 0.15s}
...
.p-center{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-size:40px;font-weight:700;opacity:0;transition:.1s;pointer-events:none}
...
.p-btn{width:44px;height:44px;background:rgba(255,255,255,.1);border:none;border-radius:50%;color:#fff;font-size:13px;font-weight:700;cursor:pointer;transition:background 0.15s}
...
<script>
(function(){
// ===== OPTIMIZACIONES DE NAVEGACIÓN RÁPIDA =====
...
const S={
    ...
    movies:[],
    focus:null,
    headerElements:[el.logo, el.srch, el.mix],
    headerIndex:0,
    currentIndex:-1,
    gridCols:4,
    cache: new Map(), // Cache de imágenes
    scrollPos: 0, // Guardar posición de scroll
    imgObserver:null,
    lastKeyTime:0 // Para debounce de teclado
};

// ===== DEBOUNCE PARA NAVEGACIÓN RÁPIDA =====
function navDebounce(fn) {
    const now = Date.now();
    if(now - S.lastKeyTime > 80) { // 80ms de debounce
        S.lastKeyTime = now;
        fn();
    }
}

// ===== GESTIÓN DE FOCUS RÁPIDA =====
function setFocusHeader(index) {
    if(index < 0) index = 0;
    if(index >= S.headerElements.length) index = S.headerElements.length - 1;
    
    // Remover focus anterior
    if(S.focus && S.focus.classList) {
        S.focus.classList.remove('f');
        if(S.focus === el.srch) el.srch.blur();
    }
...
function calculateGridColumns() {
    ...
    // Cálculo rápido usando getComputedStyle
    const style = getComputedStyle(grid);
    const cols = style.gridTemplateColumns.split(' ').length;
    S.gridCols = Math.max(1, cols || 4);
}

// ===== CARGA DE IMÁGENES OPTIMIZADA =====
function loadImageFast(img) {
    if(!img.dataset.src || img.classList.contains('loaded')) return;
    
    // Cache de imágenes
    const src = img.dataset.src;
    if(S.cache.has(src)) {
        img.src = S.cache.get(src);
    ...
    S.imgObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if(entry.isIntersecting) {
                loadImageFast(entry.target);
                S.imgObserver.unobserve(entry.target);
            }
        });
    }, { rootMargin: '200px' });
...
// ===== CARGA DE PELÍCULAS OPTIMIZADA =====
let searchTimer;
el.srch.oninput = () => {
    clearTimeout(searchTimer);
    searchTimer = setTimeout(() => loadMovies(false), 300); // Reducido a 300ms
};
...
function loadMovies(random) {
    ...
    fetch('/api/movies?limit=200' + (q ? '&q=' + encodeURIComponent(q) : '') + (random ? '&random=true' : ''))
        ...
        .then(d => {
            ...
            // Crear cards rápidamente sin delays
            const fragment = document.createDocumentFragment();
            d.data.forEach(m => {
                fragment.appendChild(mkCard(m));
            });
            el.grid.appendChild(fragment);

            // Calcular columnas y cargar imágenes
...
// Eventos del reproductor optimizados
el.vid.oncanplay = () => {
...
function showInd(t) {
    ...
    clearTimeout(indT);
    indT = setTimeout(() => el.pInd.classList.remove('show'), 400);
}
...
function init() {
    ...
    fetch('/api/movies?limit=200&random=true').then(r=>r.json()).then(d=>{
        ...
        // Crear todas las cards rápidamente
        const fragment = document.createDocumentFragment();
        d.data.forEach(m => fragment.appendChild(mkCard(m)));
        el.grid.appendChild(fragment);
...
// Recalcular columnas al redimensionar (con debounce)
let resizeTimer;
window.addEventListener('resize', () => {
    clearTimeout(resizeTimer);
    resizeTimer = setTimeout(calculateGridColumns, 100);
});
